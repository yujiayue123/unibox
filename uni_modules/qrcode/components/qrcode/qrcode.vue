...
<template>
	<view class="u-qrcode" :style="{ width: `${size}px`, height: `${size}px` }">
		...
		<canvas class="u-qrcode-canvas" :id="canvasId" :canvas-id="canvasId" :style="{
      ...
      }" @click="onClick" v-if="templateOptions.canvasDisplay"></canvas>
		<!-- #endif -->

		<!-- 微信小程序非2d模式不支持transform所以使用canvas2d -->
		<!-- #ifdef MP-WEIXIN -->
		<canvas class="u-qrcode-canvas" type="2d" :id="canvasId" :canvas-id="canvasId" :style="{
        width: `${templateOptions.canvasWidth}px`,
        height: `${templateOptions.canvasHeight}px`
      }" @click="onClick" v-if="templateOptions.canvasDisplay"></canvas>
		<!-- #endif -->
		...
		<!-- nvue用gcanvas -->
		<!-- #ifdef APP-NVUE -->
		<gcanvas class="u-qrcode-canvas" ref="gcanvas" :style="{
        width: `${templateOptions.canvasWidth}px`,
        height: `${templateOptions.canvasWidth}px`
      }" @click="onClick" v-if="templateOptions.canvasDisplay"></gcanvas>
		<!-- #endif -->
		...
		<view class="u-qrcode-makeing" v-if="makeing">
			<image class="u-qrcode-makeing-image" :style="{ width: `${size / 4}px`, height: `${size / 4}px` }"
		...
		<!-- 错误处理，可在此替换 -->
		<view class="u-qrcode-error" v-if="inError"><text class="u-qrcode-error-message">Error, see console.</text>
		</view>
	...
	export default {
			...
			make() {
				...
				this.makeDelegate = setTimeout(() => {
					...
					if (typeof this.uqrcodeOptions.errorCorrectLevel === 'string') {
						this.uqrcodeOptions.errorCorrectLevel = uQRCode.errorCorrectLevel[this.uqrcodeOptions
							.errorCorrectLevel];
					}
        ...
        Math.random()
          ...
          .split('.')[1]
				...
				const dpr = uni.getSystemInfoSync().pixelRatio;
				this.canvasContext.toTempFilePath(0, 0, this.size * dpr, this.size * dpr, this.size * dpr, this.size * dpr,
					'', 1, res => {
						callback.success(res);
