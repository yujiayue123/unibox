	...
	<view class="u-upload" v-if="!disabled">
		<view v-if="showUploadList" class="u-list-item u-preview-wrap" v-for="(item, index) in lists" :key="index"
			...
			<u-line-progress v-if="showProgress && item.progress > 0 && item.progress != 100 && !item.error"
				:show-percent="false" height="16" class="u-progress" :percent="item.progress"></u-line-progress>
		...
		<view style="display: inline-block;" @tap="selectFile" v-if="maxCount > lists.length">
			...
			<view v-if="!customBtn&&!isImg" class="u-list-item u-add-wrap" hover-class="u-add-wrap__hover"
			...
			<view v-else class="photo">
				<image :src="imgUrl" class="photo"></image>
			</view>
		</view>
	</view>
</template>
