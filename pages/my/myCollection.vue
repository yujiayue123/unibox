<template>
	<view class="uni_box myList">
		...
		<view class="list">
			<view class="item" @click="routerTo(`/pages/my/collectionDeta?type=1&id=${item.id}`)"
				...
				<view class="item_box">
					...
					<view class="deta">
						...
						<view class="code">
							...
							<view v-if="item.product_number">
								编号：{{item.product_number.number}}/{{item.product.issue_all}}</view>
						...
						<view class="btn">
							...
							<view class="btn_box" v-if="active==1">
								<view class="btn_view" v-if="market_open==1"
									@click.stop="routerTo(`/pages/my/sell?type=1&id=${item.id}`)">出售
								</view>
								<view class="btn_view" @click.stop="routerTo(`/pages/my/give?type=1&id=${item.id}`)">转赠
								</view>
							</view>
			...
			<u-modal v-model="openShow" :show-title="false" :show-confirm-button="false" :show-cancel-button="false">
				<blindBoxOpen :cover="open_cover" :name="open_name" @close="openShow=false" @reload="reload" />
			</u-modal>
		</view>
	</view>
</template>
